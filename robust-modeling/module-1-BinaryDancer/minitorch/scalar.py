...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    return (f(*[el if i != arg else el + epsilon for i, el in enumerate(vals)])
            - f(*[(el if i != arg else (el - epsilon)) for i, el in enumerate(vals)])) / 2 / epsilon
...
class Scalar(Variable):
    ...
    def __sub__(self, b):
        return Add.apply(self, operators.neg(b))
...
class Sigmoid(ScalarFunction):
    ...
    @staticmethod
    def backward(ctx, d_output):
        def _sigmoid_back(sigmoid_v, d):
            return operators.mul(d, operators.mul(sigmoid_v, operators.add(1, operators.neg(sigmoid_v))))
        a = ctx.saved_values
        return _sigmoid_back(a, d_output)
